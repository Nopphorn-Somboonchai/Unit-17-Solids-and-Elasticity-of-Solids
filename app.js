...
function getSeededRandomBase(questionId, seed, min, max, step = 1) {
    ...
    const rawVal = min + Math.floor(rng.random() * (steps + 1)) * step;
    const stepStr = step.toString();
    const decimals = stepStr.includes('.') ? stepStr.split('.')[1].length : 0;
    return decimals > 0 ? parseFloat(rawVal.toFixed(decimals)) : Math.round(rawVal);
}
...
function getOffsetFromR(r) {
    ...
    return (Math.abs(hash) % 20) + 1; // offset 1-20
}
...
const QUESTION_TEMPLATES = [
    // 17.1.1 ความเค้นตามยาว (Stress)
    {
        ...
        generate: (r) => {
            ...
            const area = r ? getSeededRandomBase('17_1_1_stress_simple_a', r, 0.5, 4.0, 0.1) : 2.0; // 0.5 - 4.0 mm²
            const f_base = r ? getSeededRandomBase('17_1_1_stress_simple_f', r, 100, 800, 10) : 400;
    ...
    },
    {
        ...
        generate: (r) => {
            const offset = getOffsetFromR(r);
            const side = r ? getSeededRandomBase('17_1_1_stress_sq_side', r, 10, 40, 2) : 20; // 10, 12, ..., 40 cm
            const f_base = r ? getSeededRandomBase('17_1_1_stress_sq_f', r, 20, 120, 2) : 50; // 20 - 120 kN
      ...
      - พื้นที่หน้าตัดสี่เหลี่ยมจัตุรัส: \\( A = \\text{ด้าน} \\times \\text{ด้าน} = ${side / 100} \\text{ m} \\times ${side / 100} \\text{ m} = ${A.toFixed(4)} \\text{ m}^2 \\)<br>
      จากสูตร: \\( \\sigma = \\frac{F}{A} \\)<br>
      \\( \\sigma = \\frac{${F_N}}{${A.toFixed(4)}} = ${formatScientificLaTeX(stress, 3)} \\text{ N/m}^2 \\)
    `
            };
        }
    },
    ...
    // 17.1.2 ความเครียดตามยาว (Strain)
    {
        ...
        generate: (r) => {
            const offset = getOffsetFromR(r);
            const l0 = r ? getSeededRandomBase('17_1_2_strain_l0', r, 1.0, 5.0, 0.2) : 2.5; // 1.0 - 5.0 m
            const dl_base = r ? getSeededRandomBase('17_1_2_strain_dl', r, 1.5, 8.0, 0.5) : 3.0; // mm
    ...
    },
    {
        ...
        inputs: [{ label: 'ความเครียดตามยาว:' }],
        text: (p) => `สายเคเบิลลิฟต์เส้นหนึ่งยืดตัวออกยาวเพิ่มขึ้นร้อยละ \\( ${p.r ? `(${p.pct_base} + \\ ${p.offset_val})` : p.pct} \\) ของความยาวเดิมเมื่อรับน้ำหนักผู้โดยสารเต็มพิกัด จงหาความเครียดตามยาวของสายเคเบิลนี้`,
        generate: (r) => {
            const offset = getOffsetFromR(r);
            const pct_base = r ? getSeededRandomBase('17_1_2_strain_pct', r, 0.05, 0.50, 0.01) : 0.20;
            const offset_val = parseFloat((offset * 0.01).toFixed(2));
            const pct = r ? parseFloat((pct_base + offset_val).toFixed(2)) : 0.20;

            const strain = pct / 100;

            return {
                params: { pct, pct_base, offset_val, r: offset },
                answers: [strain.toFixed(6), strain.toFixed(5), `\\( ${formatScientificLaTeX(strain, 2)} \\)`, `\\( ${formatScientificLaTeX(strain, 3)} \\)`, strain.toExponential(2).replace('e-', 'x10^-')],
                answersRaw: [strain],
                explanation: () => `
    ...
    // 17.1.3 มอดุลัสของยัง (Young's Modulus)
    {
        ...
        generate: (r) => {
            const offset = getOffsetFromR(r);
            const l0 = r ? getSeededRandomBase('17_1_3_ym_steel_l0', r, 1.0, 2.5, 0.1) : 1.2;
            const area = r ? getSeededRandomBase('17_1_3_ym_steel_a', r, 0.3, 1.0, 0.05) : 0.5; // mm2
            const f_base = r ? getSeededRandomBase('17_1_3_ym_steel_f', r, 80, 250, 5) : 150;
            const f = r ? f_base + offset : 150;
            const dl = r ? getSeededRandomBase('17_1_3_ym_steel_dl', r, 1.0, 3.0, 0.1) : 1.8; // mm
    ...
    },
    {
        ...
        generate: (r) => {
            const offset = getOffsetFromR(r);
            const l0 = r ? getSeededRandomBase('17_1_3_ym_find_elong_l0', r, 1.5, 6.0, 0.2) : 4.0;
            const area = r ? getSeededRandomBase('17_1_3_ym_find_elong_a', r, 0.3, 1.5, 0.05) : 0.8; // cm2
            const m_base = r ? getSeededRandomBase('17_1_3_ym_find_elong_m', r, 2000, 9000, 100) : 7000;
    ...
    // 17.1.4 ขีดความปลอดภัย (Safety Limit)
    {
        ...
        text: (p) => `ลวดโลหะชนิดหนึ่งมีขีดจำกัดสภาพยืดหยุ่นเกิดขึ้นที่ความเค้น \\( ${p.elastic_stress} \\times 10^8 \\text{ Pa} \\) ถ้าลวดเส้นนี้มีขนาดเส้นผ่านศูนย์กลาง \\( ${p.r ? `(${p.d_base} + \\ ${p.offset_val})` : p.d} \\text{ mm} \\) จะสามารถนำมาแขวนมวลวัตถุสูงสุดได้กี่กิโลกรัมโดยที่ลวดยังสามารถคืนสภาพความยาวเดิมได้อยู่ (กำหนดให้ \\( g = 10 \\text{ m/s}^2 \\))`,
        generate: (r) => {
            const offset = getOffsetFromR(r);
            const elastic_stress = r ? getSeededRandomBase('17_1_4_safety_max_mass_s', r, 1.2, 4.0, 0.1) : 2.5; // * 10^8
            const d_base = r ? getSeededRandomBase('17_1_4_safety_max_mass_d', r, 0.8, 3.0, 0.05) : 1.2;
            const offset_val = parseFloat((offset * 0.05).toFixed(2));
            const d = r ? parseFloat((d_base + offset_val).toFixed(2)) : 1.2;
            ...
            return {
                params: { elastic_stress, d, d_base, offset_val, r: offset },
    ...
    // 17.1.5 โจทย์ผสมและเปรียบเทียบอัตราส่วน (Ratios & Concepts)
    {
        ...
        generate: (r) => {
            const offset = getOffsetFromR(r);
            const a_base = r ? getSeededRandomBase('17_1_5_ratio_length_a', r, 2, 8, 1) : 3;
    ...
    // ==========================================
    // MULTIPLE CHOICE QUESTIONS (4 Questions per Topic)
    // ==========================================

    // --- TOPIC 17.1.1 (Choice 1-4) ---
    {
        ...
        choices: [
            ...
            'ความเค้นตามยาวของลวดทองแดงมีค่ามากกว่าลวดเหล็กกล้า',
    ...
    {
        id: '17_1_1_concept_stress_cross_section', topic: '17.1.1', type: 'choice',
        title: 'ผลของการเปลี่ยนขนาดเส้นผ่านศูนย์กลางต่อความเค้นตามยาว',
        choices: [
            'ลดลงเหลือ 1/4 เท่าของความเค้นเดิม',
            'ลดลงเหลือ 1/2 เท่าของความเค้นเดิม',
            'เพิ่มขึ้นเป็น 2 เท่าของความเค้นเดิม',
            'เท่าเดิมเนื่องจากออกแรงดึงขนาดเท่าเดิม'
        ],
        text: () => `ถ้าออกแรงดึงขนาดเท่าเดิมกระทำต่อลวดโลหะเส้นที่สองซึ่งมีขนาดเส้นผ่านศูนย์กลางหน้าตัดเป็น 2 เท่าของลวดเส้นแรก ความเค้นตามยาวในลวดเส้นที่สองจะเป็นอย่างไรเมื่อเทียบกับลวดเส้นแรก`,
        generate: (r) => ({
            params: {},
            answers: ['ลดลงเหลือ 1/4 เท่าของความเค้นเดิม'],
            answersRaw: [0],
            explanation: () => `
      ความเค้นตามยาวคำนวณจากสูตร: \\( \\sigma = \\frac{F}{A} \\)<br>
      พื้นที่หน้าตัดวงกลม: \\( A = \\pi \\left(\\frac{d}{2}\\right)^2 = \\frac{\\pi d^2}{4} \\) (แปรผันตรงกับกำลังสองของเส้นผ่านศูนย์กลาง \\( d^2 \\))<br>
      เมื่อเส้นผ่านศูนย์กลางเพิ่มขึ้นเป็น 2 เท่า พื้นที่หน้าตัด \\( A \\) จะเพิ่มขึ้นเป็น \\( 2^2 = 4 \\) เท่า<br>
      เนื่องจากแรงดึง \\( F \\) เท่าเดิม ความเค้นตามยาวจึงลดลงเหลือ \\( \\frac{1}{4} \\) เท่าของเดิม
    `
        })
    },
    {
        id: '17_1_1_concept_tensile_vs_compressive', topic: '17.1.1', type: 'choice',
        title: 'ชนิดของความเค้นตามยาว (ความเค้นดึงและความเค้นอัด)',
        choices: [
            'ความเค้นทั้งสองชนิดเป็นความเค้นตามยาวที่ตั้งฉากกับพื้นที่หน้าตัด โดยความเค้นดึงทำให้วัตถุยืดออก ส่วนความเค้นอัดทำให้วัตถุหดสั้นลง',
            'ความเค้นดึงมีทิศตั้งฉากกับพื้นที่หน้าตัด ส่วนความเค้นอัดมีทิศขนานกับพื้นที่หน้าตัดของวัตถุ',
            'ความเค้นดึงทำให้เกิดการเปลี่ยนรูปเฉพาะภายนอก ส่วนความเค้นอัดทำให้มวลและความหนาแน่นของเนื้อวัตถุเพิ่มขึ้น',
            'ความเค้นดึงมีหน่วยเป็นนิวตัน ส่วนความเค้นอัดมีหน่วยวัดเป็นพาสคัล'
        ],
        text: () => `ข้อใดอธิบายลักษณะของความเค้นดึง (Tensile Stress) และความเค้นอัด (Compressive Stress) ได้ถูกต้องตามหลักฟิสิกส์ที่สุด`,
        generate: (r) => ({
            params: {},
            answers: ['ความเค้นทั้งสองชนิดเป็นความเค้นตามยาวที่ตั้งฉากกับพื้นที่หน้าตัด โดยความเค้นดึงทำให้วัตถุยืดออก ส่วนความเค้นอัดทำให้วัตถุหดสั้นลง'],
            answersRaw: [0],
            explanation: () => `
      ทั้งความเค้นดึงและความเค้นอัดจัดเป็นความเค้นตามยาว (Longitudinal Stress) โดยแรงกระทำจะมีทิศตั้งฉากกับพื้นที่หน้าตัดขวางของวัตถุ ความเค้นดึงเกิดจากแรงดึงออกจากระนาบทำให้วัตถุยืดออกตามแนวยาว ส่วนความเค้นอัดเกิดจากแรงกดเข้าหาระนาบทำให้วัตถุหดสั้นลง ทั้งคู่มีหน่วยเป็น \\( \\text{N/m}^2 \\) หรือ Pa
    `
        })
    },
    {
        id: '17_1_1_concept_stress_definition', topic: '17.1.1', type: 'choice',
        title: 'นิยามความเค้นตามยาวในทางฟิสิกส์',
        choices: [
            'อัตราส่วนระหว่างแรงกระทำภายในที่ตั้งฉากกับระนาบต่อพื้นที่หน้าตัดของวัตถุ',
            'อัตราส่วนระหว่างความยาวที่เปลี่ยนไปต่อความยาวเริ่มต้นของวัตถุ',
            'ผลคูณระหว่างแรงดึงภายนอกกับพื้นที่หน้าตัดขวางของวัตถุ',
            'แรงดึงสูงสุดที่กระทำต่อเส้นลวดแล้วทำให้เส้นลวดขาดออกจากกัน'
        ],
        text: () => `ความเค้นตามยาว (Longitudinal Stress) ในทางฟิสิกส์ของแข็งหมายถึงปริมาณใด`,
        generate: (r) => ({
            params: {},
            answers: ['อัตราส่วนระหว่างแรงกระทำภายในที่ตั้งฉากกับระนาบต่อพื้นที่หน้าตัดของวัตถุ'],
            answersRaw: [0],
            explanation: () => `
      ความเค้นตามยาว (\\( \\sigma \\)) นิยามว่าเป็นอัตราส่วนระหว่างแรงตั้งฉากต่อหนึ่งหน่วยพื้นที่หน้าตัด (\\( \\sigma = \\frac{F}{A} \\)) เพื่อบอกระดับแรงต้านทานภายในเนื้อวัตถุต่อการเปลี่ยนรูปทรง
    `
        })
    },

    // --- TOPIC 17.1.2 (Choice 1-4) ---
    {
    ...
    {
        id: '17_1_2_concept_strain_definition', topic: '17.1.2', type: 'choice',
        title: 'ความหมายเชิงกายภาพของความเครียดตามยาว',
        choices: [
            'อัตราส่วนระหว่างความยาวที่เปลี่ยนไปต่อความยาวเดิม เป็นปริมาณไร้มิติที่บอกระดับการเสียรูปทรง',
            'อัตราส่วนระหว่างแรงดึงต่อความยาวเดิม มีหน่วยเป็นนิวตันต่อเมตร',
            'ระยะที่วัตถุยืดออกจริง มีหน่วยเป็นเมตรหรือมิลลิเมตร',
            'ผลคูณระหว่างความยาวเดิมกับความเค้นตามยาว มีหน่วยเป็นพาสคัลเมตร'
        ],
        text: () => `ความเครียดตามยาว (Longitudinal Strain) ในทางฟิสิกส์มีความหมายเชิงกายภาพและลักษณะอย่างไร`,
        generate: (r) => ({
            params: {},
            answers: ['อัตราส่วนระหว่างความยาวที่เปลี่ยนไปต่อความยาวเดิม เป็นปริมาณไร้มิติที่บอกระดับการเสียรูปทรง'],
            answersRaw: [0],
            explanation: () => `
      ความเครียดตามยาว (\\( \\varepsilon = \\frac{\\Delta L}{L_0} \\)) เป็นปริมาณที่แสดงอัตราส่วนการยืดหรือหดของความยาวเทียบกับความยาวเดิม เพื่อวัดการเสียรูปตามแนวยาว เป็นปริมาณไม่มีหน่วย (Dimensionless)
    `
        })
    },
    {
        id: '17_1_2_concept_strain_comparison', topic: '17.1.2', type: 'choice',
        title: 'เปรียบเทียบความเครียดของลวดที่มีความยาวต่างกัน',
        choices: [
            'ความเครียดตามยาวของลวด A เป็น 2 เท่าของลวด B',
            'ความเครียดตามยาวของลวด B เป็น 2 เท่าของลวด A',
            'ความเครียดตามยาวของลวดทั้งสองเท่ากันเพราะยืดออก 2 มิลลิเมตรเท่ากัน',
            'ความเครียดตามยาวของลวด A เป็น 4 เท่าของลวด B'
        ],
        text: () => `ลวดเหล็กกล้า A ยาวเริ่มต้น 1 เมตร และลวดเหล็กกล้า B ยาวเริ่มต้น 2 เมตร ถูกดึงจนแต่ละเส้นยืดออกยาวขึ้น 2 มิลลิเมตรเท่ากัน ข้อใดเปรียบเทียบความเครียดตามยาวของลวดทั้งสองได้ถูกต้อง`,
        generate: (r) => ({
            params: {},
            answers: ['ความเครียดตามยาวของลวด A เป็น 2 เท่าของลวด B'],
            answersRaw: [0],
            explanation: () => `
      จากสมการความเครียด: \\( \\varepsilon = \\frac{\\Delta L}{L_0} \\)<br>
      - ลวด A: \\( \\varepsilon_A = \\frac{2 \\times 10^{-3}}{1} = 2 \\times 10^{-3} \\)<br>
      - ลวด B: \\( \\varepsilon_B = \\frac{2 \\times 10^{-3}}{2} = 1 \\times 10^{-3} \\)<br>
      ดังนั้น \\( \\varepsilon_A = 2 \\varepsilon_B \\) ความเครียดของลวด A จึงเป็น 2 เท่าของลวด B
    `
        })
    },
    {
        id: '17_1_2_concept_shear_volume_strain', topic: '17.1.2', type: 'choice',
        title: 'การจำแนกประเภทความเครียดในของแข็ง',
        choices: [
            'ความเครียดเชิงปริมาตร (Volume Strain)',
            'ความเครียดตามยาว (Tensile Strain)',
            'ความเครียดเฉือน (Shear Strain)',
            'ความเครียดพลาสติก (Plastic Strain)'
        ],
        text: () => `เมื่อนำวัตถุของแข็งก้อนหนึ่งไปหย่อนลงในของเหลวใต้ทะเลลึก ความดันของของเหลวกระทำต่อวัตถุทุกทิศทางทำให้วัตถุมีขนาดเล็กลงแต่ยังคงรูปทรงเดิม การเสียรูปนี้ทำให้เกิดความเครียดประเภทใด`,
        generate: (r) => ({
            params: {},
            answers: ['ความเครียดเชิงปริมาตร (Volume Strain)'],
            answersRaw: [0],
            explanation: () => `
      เมื่อวัตถุถูกแรงกระทำรอบด้านอย่างสม่ำเสมอทำให้เกิดการเปลี่ยนแปลงของปริมาตรต่อปริมาตรเดิม (\\( \\frac{\\Delta V}{V_0} \\)) โดยที่รูปทรงยังคงเดิม เรียกว่า ความเครียดเชิงปริมาตร (Volume Strain หรือ Bulk Strain)
    `
        })
    },

    // --- TOPIC 17.1.3 (Choice 1-4) ---
    {
        ...
        text: () => `ลวดสองเส้นทำด้วยวัสดุคนละชนิดกัน แต่มีความยาวเริ่มต้นและขนาดเส้นผ่านศูนย์กลางเท่ากันทุกประการ เมื่อนำมาแขวนวัตถุมวลเท่ากันในแนวดิ่ง ข้อใดอธิบายการยืดตัวของลวดทั้งสองเมื่อเปรียบเทียบกับค่ามอดุลัสของยัง (Young's Modulus) ได้ถูกต้องที่สุด`,
        generate: (r) => ({
            params: {},
    ...
    {
        id: '17_1_3_concept_ym_factors', topic: '17.1.3', type: 'choice',
        title: 'ปัจจัยที่มีผลต่อค่ามอดุลัสของยัง',
        choices: [
            'ชนิดของสารและอุณหภูมิของวัตถุเท่านั้น ไม่ขึ้นกับขนาดรูปทรงหรือแรงดึงภายนอก',
            'ขนาดความยาวเริ่มต้นและพื้นที่หน้าตัดของเส้นลวด',
            'ขนาดของแรงดึงภายนอกที่กระทำต่อเส้นลวด',
            'ระยะยืดสูงสุดที่เส้นลวดสามารถทนทานได้ก่อนขาด'
        ],
        text: () => `ค่ามอดุลัสของยัง (Young's Modulus) ของลวดโลหะขึ้นอยู่กับปัจจัยใด`,
        generate: (r) => ({
            params: {},
            answers: ['ชนิดของสารและอุณหภูมิของวัตถุเท่านั้น ไม่ขึ้นกับขนาดรูปทรงหรือแรงดึงภายนอก'],
            answersRaw: [0],
            explanation: () => `
      มอดุลัสของยัง (\\( Y \\)) เป็นสมบัติเฉพาะตัวของสาร (Intensive Material Property) ขึ้นอยู่กับแรงยึดเหนี่ยวระหว่างอะตอมของเนื้อวัสดุและอุณหภูมิเท่านั้น ค่านี้จะไม่เปลี่ยนแปลงตามความยาว พื้นที่หน้าตัด หรือขนาดของแรงภายนอกที่มากระทำ
    `
        })
    },
    {
        id: '17_1_3_concept_ym_unit_dimension', topic: '17.1.3', type: 'choice',
        title: 'หน่วยและมิติของมอดุลัสของยัง',
        choices: [
            'ความดันและความเค้นตามยาว (มีหน่วยเป็น N/m² หรือ Pa)',
            'พลังงานและงานกล (มีหน่วยเป็น จูล)',
            'แรงดึง (มีหน่วยเป็น นิวตัน)',
            'เป็นปริมาณที่ไม่มีหน่วยวัดเช่นเดียวกับความเครียด'
        ],
        text: () => `มอดุลัสของยัง (Young's Modulus) มีหน่วยวัดในระบบเอสไอ (SI) ตรงกับหน่วยวัดของปริมาณใดในวิชาฟิสิกส์`,
        generate: (r) => ({
            params: {},
            answers: ['ความดันและความเค้นตามยาว (มีหน่วยเป็น N/m² หรือ Pa)'],
            answersRaw: [0],
            explanation: () => `
      จากสูตร \\( Y = \\frac{\\sigma}{\\varepsilon} \\) เนื่องจากความเครียด (\\( \\varepsilon \\)) เป็นปริมาณไร้มิติไม่มีหน่วย หน่วยของมอดุลัสของยังจึงตรงกับหน่วยของความเค้น (\\( \\sigma \\)) และความดัน (\\( P \\)) คือ นิวตันต่อตารางเมตร (\\( \\text{N/m}^2 \\)) หรือ พาสคัล (\\( \\text{Pa} \\))
    `
        })
    },
    {
        id: '17_1_3_concept_ym_hookes_law', topic: '17.1.3', type: 'choice',
        title: 'กฎของฮุกและขีดจำกัดการแปรผันตรง',
        choices: [
            'ความเค้นแปรผันตรงกับความเครียด เป็นจริงเฉพาะในช่วงที่ไม่เกินขีดจำกัดการแปรผันตรง',
            'ความเค้นแปรผกผันกับความเครียด เป็นจริงตั้งแต่จุดเริ่มต้นจนกระทั่งวัสดุแตกหัก',
            'ความเค้นมีค่าคงที่เสมอไม่ขึ้นกับความเครียด เป็นจริงเฉพาะในช่วงพลาสติก',
            'ความเค้นแปรผันตรงกับกำลังสองของความเครียด เป็นจริงจนถึงจุดคราก'
        ],
        text: () => `ตามกฎของฮุก (Hooke's Law) ความสัมพันธ์ระหว่างความเค้นตามยาวและความเครียดตามยาวเป็นอย่างไร และความสัมพันธ์นี้เป็นจริงในช่วงใดของการดึงวัตถุ`,
        generate: (r) => ({
            params: {},
            answers: ['ความเค้นแปรผันตรงกับความเครียด เป็นจริงเฉพาะในช่วงที่ไม่เกินขีดจำกัดการแปรผันตรง'],
            answersRaw: [0],
            explanation: () => `
      กฎของฮุกกล่าวว่าความเค้นแปรผันตรงกับความเครียด (\\( \\sigma = Y \\varepsilon \\)) โดยมีมอดุลัสของยังเป็นค่าคงที่ความชัน กฎนี้เป็นจริงเฉพาะในช่วงขีดจำกัดการแปรผันตรง (Proportional Limit) เท่านั้น
    `
        })
    },

    // --- TOPIC 17.1.4 (Choice 1-4) ---
    {
    ...
    {
        id: '17_1_4_concept_elastic_vs_plastic', topic: '17.1.4', type: 'choice',
        title: 'ความแตกต่างระหว่างสภาพยืดหยุ่นและสภาพพลาสติก',
        choices: [
            'วัตถุจะไม่สามารถคืนสู่รูปทรงเดิมได้ โดยจะเกิดการยืดตัวเสียรูปอย่างถาวร (Plastic Deformation)',
            'วัตถุจะหดกลับคืนสู่ความยาวเดิมได้สมบูรณ์ทันทีที่หยุดออกแรง',
            'วัตถุจะขาดสะบั้นออกจากกันทันทีโดยไม่เกิดการเปลี่ยนรูปร่างใดๆ',
            'ค่ามอดุลัสของยังของวัตถุจะเพิ่มขึ้นเป็นสองเท่าเพื่อต่อต้านแรงดึง'
        ],
        text: () => `เมื่อออกแรงดึงเส้นลวดโลหะจนความเค้นเกินขีดจำกัดสภาพยืดหยุ่น (Elastic Limit) แล้วปล่อยมือ วัตถุจะมีพฤติกรรมอย่างไรตามหลักฟิสิกส์`,
        generate: (r) => ({
            params: {},
            answers: ['วัตถุจะไม่สามารถคืนสู่รูปทรงเดิมได้ โดยจะเกิดการยืดตัวเสียรูปอย่างถาวร (Plastic Deformation)'],
            answersRaw: [0],
            explanation: () => `
      เมื่อแรงดึงกระทำจนความเค้นเกินขีดจำกัดสภาพยืดหยุ่น (Elastic Limit) วัสดุจะสูญเสียความสามารถในการหดตัวกลับคืนสู่ขนาดและความยาวเดิม เรียกว่าเกิดการเสียรูปถาวรหรือการเสียรูปในสภาพพลาสติก (Plastic Deformation)
    `
        })
    },
    {
        id: '17_1_4_concept_ultimate_tensile_strength', topic: '17.1.4', type: 'choice',
        title: 'ความหมายของความเค้นดึงสูงสุด (Ultimate Tensile Strength)',
        choices: [
            'ความเค้นสูงสุดที่วัสดุทนรับได้ก่อนเริ่มเกิดการคอดกิ่วและแตกหักเสียหาย',
            'ความเค้นสูงสุดที่วัสดุยังคงหดกลับคืนรูปเดิมได้ 100%',
            'ความเค้นที่ทำให้กราฟมีความชันเป็นศูนย์และวัสดุเปลี่ยนเป็นของเหลว',
            'ความเค้นต่ำสุดที่เริ่มทำให้เกิดการสั่นพ้องในเนื้อสาร'
        ],
        text: () => `จุดสูงสุดของเส้นกราฟความเค้น-ความเครียด (Ultimate Tensile Strength: UTS) บ่งชี้ถึงคุณสมบัติใดของวัสดุ`,
        generate: (r) => ({
            params: {},
            answers: ['ความเค้นสูงสุดที่วัสดุทนรับได้ก่อนเริ่มเกิดการคอดกิ่วและแตกหักเสียหาย'],
            answersRaw: [0],
            explanation: () => `
      จุด Ultimate Tensile Strength (UTS) หรือความเค้นดึงสูงสุด คือค่าความเค้นสูงสุดที่เนื้อวัสดุสามารถต้านทานแรงดึงได้ก่อนที่ชิ้นงานจะเริ่มเกิดการคอดกิ่ว (Necking) และนำไปสู่จุดแตกหัก (Fracture Point) ในที่สุด
    `
        })
    },
    {
        id: '17_1_4_concept_brittle_vs_ductile', topic: '17.1.4', type: 'choice',
        title: 'พฤติกรรมการรับแรงดึงของวัสดุเปราะเทียบกับวัสดุเหนียว',
        choices: [
            'วัสดุเปราะจะแตกหักทันทีเมื่อพ้นช่วงยืดหยุ่น โดยแทบไม่มีช่วงการเสียรูปถาวรแบบพลาสติก',
            'วัสดุเปราะจะยืดตัวเป็นเส้นยาวได้มากกว่าวัสดุเหนียวก่อนจะแตกหัก',
            'วัสดุเปราะจะมีค่ามอดุลัสของยังเป็นศูนย์เสมอในทุกช่วงของการทดสอบ',
            'วัสดุเปราะจะไม่เกิดความเค้นและความเครียดเลยเมื่อได้รับแรงกระทำ'
        ],
        text: () => `ในการทดสอบดึงเปรียบเทียบระหว่างแก้ว (วัสดุเปราะ) กับทองแดง (วัสดุเหนียว) ข้อใดอธิบายพฤติกรรมของวัสดุเปราะได้ถูกต้องที่สุด`,
        generate: (r) => ({
            params: {},
            answers: ['วัสดุเปราะจะแตกหักทันทีเมื่อพ้นช่วงยืดหยุ่น โดยแทบไม่มีช่วงการเสียรูปถาวรแบบพลาสติก'],
            answersRaw: [0],
            explanation: () => `
      วัสดุเปราะ (Brittle Material) เช่น แก้ว เซรามิก เมื่อได้รับความเค้นเกินขีดจำกัดยืดหยุ่นจะแตกหัก (Fracture) ในทันทีโดยไม่มีช่วงพลาสติก ส่วนวัสดุเหนียว (Ductile Material) เช่น โลหะทองแดง จะมีช่วงพลาสติกที่กว้าง สามารถยืดตัวเป็นเส้นยาวได้ก่อนจะขาด
    `
        })
    },

    // --- TOPIC 17.1.5 (Choice 1-4) ---
    {
    ...
    {
        id: '17_1_5_concept_graph_area', topic: '17.1.5', type: 'choice',
        title: 'ความหมายของพื้นที่ใต้กราฟแรงกับระยะยืด',
        choices: [
            'พลังงานศักย์ยืดหยุ่นที่สะสมในวัตถุ (หรือ งานที่ใช้ในการดึงวัตถุ)',
            'ค่ามอดุลัสของยังของวัสดุ',
            'ความเค้นดึงสูงสุดที่จุดแตกหัก',
            'อัตราส่วนระหว่างพื้นที่หน้าตัดต่อความยาวเริ่มต้น'
        ],
        text: () => `พื้นที่ใต้กราฟความสัมพันธ์ระหว่างแรงดึง (F) กับระยะยืด (\\(\\Delta L\\)) ในช่วงสภาพยืดหยุ่น แสดงถึงปริมาณใดในทางฟิสิกส์`,
        generate: (r) => ({
            params: {},
            answers: ['พลังงานศักย์ยืดหยุ่นที่สะสมในวัตถุ (หรือ งานที่ใช้ในการดึงวัตถุ)'],
            answersRaw: [0],
            explanation: () => `
      พื้นที่ใต้กราฟแรงดึง (F) กับระยะยืด (\\( \\Delta L \\)) คำนวณจาก \\( W = \\frac{1}{2} F \\Delta L \\) ซึ่งคืองานที่ใช้ในการดึงวัตถุให้ยืดออก หรือเท่ากับพลังงานศักย์ยืดหยุ่น (Elastic Potential Energy, \\( U_s \\)) ที่ถูกกักเก็บไว้ในเนื้อสาร
    `
        })
    },
    {
        id: '17_1_5_concept_graph_points_order', topic: '17.1.5', type: 'choice',
        title: 'ลำดับจุดสำคัญบนกราฟความเค้น-ความเครียด',
        choices: [
            'ขีดจำกัดการแปรผันตรง -> ขีดจำกัดสภาพยืดหยุ่น -> จุดคราก -> จุดความเค้นสูงสุด -> จุดแตกหัก',
            'ขีดจำกัดสภาพยืดหยุ่น -> ขีดจำกัดการแปรผันตรง -> จุดความเค้นสูงสุด -> จุดคราก -> จุดแตกหัก',
            'จุดคราก -> ขีดจำกัดการแปรผันตรง -> ขีดจำกัดสภาพยืดหยุ่น -> จุดแตกหัก -> จุดความเค้นสูงสุด',
            'ขีดจำกัดการแปรผันตรง -> จุดคราก -> ขีดจำกัดสภาพยืดหยุ่น -> จุดแตกหัก -> จุดความเค้นสูงสุด'
        ],
        text: () => `เมื่อนำแท่งโลหะเหนียวมาทำการทดสอบดึงจนขาดออกจากกัน ข้อใดเรียงลำดับจุดสำคัญที่เกิดขึ้นบนกราฟความเค้น-ความเครียดได้อย่างถูกต้อง`,
        generate: (r) => ({
            params: {},
            answers: ['ขีดจำกัดการแปรผันตรง -> ขีดจำกัดสภาพยืดหยุ่น -> จุดคราก -> จุดความเค้นสูงสุด -> จุดแตกหัก'],
            answersRaw: [0],
            explanation: () => `
      ลำดับพฤติกรรมบนกราฟ Stress-Strain ของโลหะเหนียวเรียงตามลำดับคือ: 1. ขีดจำกัดการแปรผันตรง (Proportional Limit) -> 2. ขีดจำกัดสภาพยืดหยุ่น (Elastic Limit) -> 3. จุดคราก (Yield Point) -> 4. จุดความเค้นดึงสูงสุด (Ultimate Tensile Strength) -> 5. จุดแตกหัก (Fracture/Breaking Point)
    `
        })
    }
];
...
for (let i = 1; i <= 13; i++) {
    ...
    if (i % 3 === 1) {
        QUESTION_TEMPLATES.push({
            ...
            generate: (r) => {
                const offset = getOffsetFromR(r);
                const dia = r ? getSeededRandomBase(`17_1_1_gen_d_${i}`, r, 0.6, 2.5, 0.05) : 1.0;
                const m_base = r ? getSeededRandomBase(`17_1_1_gen_m_${i}`, r, 30, 300, 5) : 100;
        ...
        QUESTION_TEMPLATES.push({
            ...
            inputs: [{ label: 'ความเครียดตามยาว:' }],
            text: (p) => `เสาคอนกรีตสูง \\( ${p.l0} \\text{ m} \\) ย่นหดตัวลงไปจากเดิม \\( ${p.r ? `(${p.dl_base} + \\ ${p.offset_val})` : p.dl} \\text{ mm} \\) เมื่อมีแรงกดทับอย่างหนักจากด้านบน จงหาขนาดของความเครียดตามยาวในเสาต้นนี้`,
            generate: (r) => {
                const offset = getOffsetFromR(r);
                const l0 = r ? getSeededRandomBase(`17_1_2_gen_l0_${i}`, r, 2.0, 8.0, 0.2) : 4.0;
                const dl_base = r ? getSeededRandomBase(`17_1_2_gen_dl_${i}`, r, 0.2, 2.0, 0.05) : 0.8;
                const offset_val = parseFloat((offset * 0.05).toFixed(2));
                const dl = r ? parseFloat((dl_base + offset_val).toFixed(2)) : 0.8;

                const dl_m = dl / 1000;
                const strain = dl_m / l0;

                return {
                    params: { l0, dl, dl_base, offset_val, r: offset },
        ...
        QUESTION_TEMPLATES.push({
            ...
            generate: (r) => {
                const offset = getOffsetFromR(r);
                const l0 = r ? getSeededRandomBase(`17_1_3_gen_l0_${i}`, r, 1.0, 4.0, 0.1) : 2.0;
                const area = r ? getSeededRandomBase(`17_1_3_gen_a_${i}`, r, 0.3, 2.0, 0.05) : 1.0;
                const f_base = r ? getSeededRandomBase(`17_1_3_gen_f_${i}`, r, 100, 500, 5) : 300;
...
// Add extra variations to round up questions of different topics
QUESTION_TEMPLATES.push(
    {
        ...
        text: (p) => `เสาหินมีค่าขีดจำกัดการทนความเค้นสูงสุดก่อนที่มันจะแตกร้าวหรือชำรุดเสียหายที่ \\( ${p.max_stress} \\times 10^7 \\text{ Pa} \\) ถ้าวิศวกรออกแบบระบบก่อสร้างเสานี้โดยกำหนดค่าตัวคูณความปลอดภัย (Safety Factor) เท่ากับ \\( ${p.r ? `(${p.sf_base} + \\ ${p.offset_val})` : p.sf} \\) จงหาขีดจำกัดของความเค้นดึงสูงสุดที่ปลอดภัยต่อการใช้งานในหน่วยพาสคัล`,
        generate: (r) => {
            const offset = getOffsetFromR(r);
            const max_stress = r ? getSeededRandomBase('17_1_4_safety_fac_ms', r, 4.0, 16.0, 0.5) : 8.0; // * 10^7
            const sf_base = r ? getSeededRandomBase('17_1_4_safety_fac_sf', r, 1.8, 4.5, 0.1) : 2.5;
            const offset_val = parseFloat((offset * 0.05).toFixed(2));
            const sf = r ? parseFloat((sf_base + offset_val).toFixed(2)) : 2.5;

            const safe_stress = (max_stress * 1e7) / sf;

            return {
                params: { max_stress, sf, sf_base, offset_val, r: offset },
    ...
    },
    {
        ...
        text: (p) => `ลวดโลหะ X มีมอดุลัสของยังเป็น \\( ${p.Yx} \\times 10^{11} \\text{ Pa} \\) และลวดโลหะ Y มีมอดุลัสของยังเป็น \\( ${p.Yy} \\times 10^{11} \\text{ Pa} \\) ถ้าลวดทั้งสองมีความยาวเริ่มต้นและพื้นที่หน้าตัดเท่ากัน เมื่อดึงด้วยแรงเท่ากัน จงหาอัตราส่วนของระยะยืดลวด X ต่อระยะยืดลวด Y (ทศนิยม 2 ตำแหน่ง)`,
        generate: (r) => {
            const offset = getOffsetFromR(r);
            const Yx_base = r ? getSeededRandomBase('17_1_5_comp_yx', r, 1.8, 3.2, 0.1) : 2.0;
            const Yy_base = r ? getSeededRandomBase('17_1_5_comp_yy', r, 0.8, 1.6, 0.1) : 1.1;
            const Yx = parseFloat(Yx_base.toFixed(1));
            const Yy = parseFloat(Yy_base.toFixed(1));
            // dLx / dLy = Yy / Yx (inversely proportional)
            const ratio = Yy / Yx;

            return {
                params: { Yx, Yy, r: offset },
                answers: [ratio.toFixed(2), ratio.toFixed(3)],
                answersRaw: [ratio],
                explanation: () => `
      ...
      \\( \\frac{\\Delta L_X}{\\Delta L_Y} = \\frac{Y_Y}{Y_X} = \\frac{${Yy} \\times 10^{11} \\text{ Pa}}{${Yx} \\times 10^{11} \\text{ Pa}} = ${ratio.toFixed(2)} \\) เท่า
    `
            };
        }
    }
...
function startExamProcess() {
    ...
    // Choose one topic randomly to be the choice question using SeededRNG
    const choiceTopicIndex = Math.floor(examRNG.random() * topics.length);
    ...
    currentExamQuestions = selectedTemplates.map((template, index) => {
        ...
        const choiceRNG = new SeededRNG(`${num}_${timestamp}_${template.id}_choices`);
        const choices = template.type === 'choice' ? choiceRNG.shuffle(template.choices) : [];
        return {
